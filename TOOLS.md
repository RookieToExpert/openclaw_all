...
| 创建训练 / 推理任务                                              | `skills/job-create/SKILL.md`             | `tools/job-templates.md`；`tools/rayctl-kubectl.md`        |
...
| 查看 D 集群物理机目录 / 日志 / NPU / 磁盘 / DNS / 宿主机训练网互通        | `skills/dcluster-machine-op/SKILL.md`    | `tools/dcluster-ansible.md`；必要时先读 `tools/rayctl-kubectl.md` → 4.1 节点查询 |
...
| 重启 / 重置 VC 控制面、删除 HC 下 VC 控制面 Pod 触发重建 | `skills/hc-system-op/SKILL.md` | `tools/rayctl-kubectl.md` → 4.2 平台 VC 信息查询；`tools/hc-system-kubectl.md` |
| 扩容 VC / 调整 VC flavor / 修改 VC 规格 minimum、small、medium、large、unlimit | `skills/hc-system-op/SKILL.md` | `tools/hc-system-kubectl.md` |
| 给某个 VC 开放特权模式 / 查询或更新 `disallow-privileged-containers` policy 例外 | `skills/hc-system-op/SKILL.md` | `tools/rayctl-kubectl.md` → `rayctl vc get`、policy get / update；`tools/hc-system-kubectl.md` → 8. `disallow-privileged-containers` policy 更新模板 |
